...
norm <- glmmTMB(gf ~ -1 + (0 + atk | team_name) + (0 + def | opponent) + home, 
                data = df_train, 
...
# Previsões para gols marcados (gf_hat)
df_train$gf_hat <- predict(norm, df_train, type = "response")
...
#out = knn(train = df_train2, test = df_test2, cl = 'Y')
...
res = simulateResiduals(norm, method = "PIT")
eps = res$scaledResiduals

H = 10
hist(eps, breaks = H, probability = TRUE)
abline(h = 1, lty = 2, col = 'red')

q_eps = qnorm(eps)
q_eps = q_eps[which(q_eps != Inf)]
qqnorm(q_eps)
qqline(q_eps)
