...
#               control = control
#               )
#saveRDS(fit1, "results/poisson.rds")
fit1 = readRDS("results/poisson.rds")
```

```{r}
#model2 = stan_model("models/poisson_dependence.stan", model_name = "poisson_dependence")
#
...
#               control = control
#               )
#saveRDS(fit2, "results/poisson_dependence.rds")
fit2 = readRDS("results/poisson_dependence.rds")
```

```{r}
#model3 = stan_model("models/zipoisson.stan", model_name = "zipoisson")
#
...
#               control = control
#               )
#saveRDS(fit3, "results/zipoisson.rds")
fit3 = readRDS("results/zipoisson.rds")
```

```{r}
#model4 = stan_model("models/poisson_gamma.stan", model_name = "poisson_gamma")
#
...
#               control = control
#               )
#saveRDS(fit4, "results/poisson_gamma.rds")
fit4 = readRDS("results/poisson_gamma.rds")
```


```{r}
print(fit4, pars=c("w_att"))
```
...
```{r}
traceplot(fit4, inc_warmup = TRUE, pars = c("att"))
```
...
```{r}
matrix1 = predict_games(114, 0:5, fit1)
matrix2 = predict_games(114, 0:5, fit2)
matrix3 = predict_games(114, 0:5, fit3)
matrix4 = predict_games(114, 0:5, fit4)
matrix1
matrix2
matrix3
matrix4
...
sum(matrix2[lower.tri(matrix2)])
sum(diag(matrix2))
sum(matrix2[upper.tri(matrix2)])

sum(matrix3[lower.tri(matrix3)])
sum(diag(matrix3))
sum(matrix3[upper.tri(matrix3)])

sum(matrix4[lower.tri(matrix4)])
sum(diag(matrix4))
sum(matrix4[upper.tri(matrix4)])
```
...
df_predicted2 = predict_league(fit2, df)
df_predicted3 = predict_league(fit3, df)
df_predicted4 = predict_league(fit4, df)
```


```{r}
teste = df_predicted1[[1]] %>%
  filter(round == 38)

teste2 = df_predicted2[[1]] %>%
  filter(round == 38)

teste3 = df_predicted3[[1]] %>%
  filter(round == 38)

teste4 = df_predicted4[[1]] %>%
  filter(round == 38)
```
